...
const FALLBACK_SUBTITLE = "Please read these Terms carefully before using Ayurmuni";
...
function parseHeading(block) {
  ...
  const explicit = String(block?.number || "").trim();
  const match = raw.match(/^(\d+)\.\s+(.*)$/);

  if (explicit) return { number: explicit, text: raw };
...
function isContactish(text) {
  return /email\s*:|phone\s*:|contact number|website\s*:|@|private limited|floor|sector|gurugram|haryana|\+?\d[\d\s-]{8,}/i.test(
    String(text || ""),
  );
}

function isAllCapsCallout(text) {
...
function RichText({ text }) {
  ...
  if (labeled && !/^https?:/i.test(labeled[1])) {
    ...
    const isWebsite = /website/i.test(label) && /^[a-z0-9.-]+\.[a-z]{2,}$/i.test(rest);
...
function contactIcon(text) {
  ...
  if (/phone\s*:|contact number/i.test(value) || /^[+\d][\d\s-]{8,}$/.test(value)) return Phone;
  if (/website\s*:/i.test(value)) return Globe;
...
function PolicyBlock({ block, index }) {
  ...
  if (block.type === "heading") {
    ...
    const id = slugify(heading.text, index);
  ...
  if (block.type === "list") {
    ...
    const showMarkers = items.some((item) => String(item?.marker || "").trim());
    const ordered = !showMarkers && isSequentialList(items);
      ...
      <ListTag className={`space-y-2.5 ${ordered ? "list-decimal pl-5" : "pl-0"}`}>
        {items.map((item, itemIndex) => {
          ...
          return (
            ...
            >
              {!ordered && marker ? (
                <span className="mt-0.5 w-8 shrink-0 text-[13px] font-semibold tabular-nums text-[#0D614E]/70">
                  {marker}
                </span>
              ) : null}
              {!ordered && !marker ? (
  ...
  if (block.type === "note") {
    return (
      <div className="rounded-2xl border border-[#cfe6dc] bg-[#f4faf7] px-4 py-4 text-[15px] leading-7 text-slate-700 md:text-base">
  ...
  if (block.type === "text" || block.type === "paragraph" || block.text) {
    ...
    if (isAllCapsCallout(text)) {
      return (
        <p className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold tracking-wide text-amber-950">
          {text}
        </p>
      );
    }
    ...
    return (
      <p className="flex items-start gap-3 text-[15px] leading-[1.8] text-slate-700 md:text-base md:leading-[1.85]">
        {block.number ? (
          <span className="mt-0.5 w-8 shrink-0 font-sans text-[13px] font-semibold tabular-nums text-[#0D614E]/55">
            {block.number}
          </span>
        ) : null}
...
export default function TermsOfUsePage() {
  ...
  const loadPolicy = useCallback(async () => {
    try {
      ...
      setPolicy(null);
      setError(err?.message || "Unable to load the Terms of Use right now.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const headings = useMemo(
    () =>
      grouped.sections.map((section) => {
        ...
        return {
          id: slugify(heading.text, section.index),
          text: heading.text,
              ...
              <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1fr)_240px]">
                ...
                >
                  ...
                  <div className="divide-y divide-[#eef4f1] px-5 md:px-12">
                    {grouped.sections.map((section) => (
                      <section
                        key={slugify(parseHeading(section.heading).text, section.index)}
